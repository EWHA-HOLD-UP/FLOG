...
class _QpuzzleScreenState extends State<QpuzzleScreen> {
  ...
  Widget puzzleBody() {
    if (image != null) { //이미지가 선택되었다면
                        ...
                        GestureDetector(
                          onTap: () {
                            if (!unlockStates[row * 2 + col] && isQuestionSheetShowed == false || selectedCellIndex == row * 2 + col) {
                              ...
                              if(selectedCellIndex != row * 2 + col) { //만약 새로운 조각 클릭 시,
                          ...
                          child: Container( //분할된 조각
                            ...
                            decoration: BoxDecoration(
                              ...
                              borderRadius: BorderRadius.only( //둥근 테두리 설정
                                topLeft: Radius.circular(
                                    (row == 0 && col == 0) ? 23.0 : 0.0), // 1행 1열 - 좌측 상단 모서리
                                topRight: Radius.circular(
                                    (row == 0 && col == 1) ? 23.0 : 0.0), // 1행 2열 - 우측 상단 모서리
                                bottomLeft: Radius.circular(
                                    (row == 2 && col == 0) ? 23.0 : 0.0), // 3행 1열 - 좌측 하단 모서리
                                      ...
                                      Container(
                                        decoration: BoxDecoration(
                                          ...
                                          borderRadius: BorderRadius.only( //둥근 모서리
                                            topLeft: Radius.circular(
                                                (row == 0 && col == 0) ? 23.0 : 0.0), // 1행 1열
                                            topRight: Radius.circular(
                                                (row == 0 && col == 1) ? 23.0 : 0.0), // 1행 2열
                                            bottomLeft: Radius.circular(
                                                (row == 2 && col == 0) ? 23.0 : 0.0), // 3행 1열
                                            bottomRight: Radius.circular(
      ...
      return Container(
        ...
        child: Center(
          child: InkWell( //+버튼 누르면 앨범에서 사진 선택 가능
            onTap: () {
              onPickImage(); //갤러리에서 사진 선택하여 불러오는 함수
            },
            child: Image.asset(
                "button/plus.png", //추후에 이미지+ 버튼 제작하여 변경?
                width: 30,
                height: 30
            ),
          ),
  ...
  void onPickImage() async {
    ...
    final croppedImage = await ImageCropper() //2:3 비율로 크롭
        .cropImage(sourcePath: image!.path, aspectRatio: const CropAspectRatio(ratioX: 2, ratioY: 3));
    setState(() {
                  ...
                  children: [
                    ...
                    Center(
                      child: Padding(
                        ...
                        child: Text( //💚 DB에서 인덱스 활용하여 질문 따오기
                    ...
                    ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int rowIndex) {
                        return GestureDetector(
                          onTap: () {
                            if(memberStatus[rowIndex] == 2 && isAnswered == false) { //'나'의 박스: '답변 작성하기' 부분을 클릭하면
                              showAnswerSheet(context); //답변창 나타남
                            }
                          },
                          child : Container( //구성원 각각의 답변 상태 or 답변이 나타나는 상자
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Container(
                                    ...
                                    child: Center( //💚 나중에 사람마다 다른 프사로 받아와서 띄울 수 있게 수정하기
                                      child: ClipRRect(
                                        borderRadius: BorderRadius.circular(50), //경계
                                        child: Image.asset( //프로필 사진 💚 나중에 파이어베이스에서 받아오기
  ...
  void showAnswerSheet(BuildContext context){
    showModalBottomSheet(
        ...
        shape: const RoundedRectangleBorder(
          borderRadius: BorderRadius.vertical(top: Radius.circular(20.0)), //위쪽 둥근 모서리
        ),
        builder: (BuildContext context) {
          return Padding(
                      ...
                      children: [
                        ...
                        Row(
                            children: [
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  child: Center(
                                    child: ClipRRect(
                                      borderRadius: BorderRadius.circular(50), //경계
                                      child: Image.asset( //내 프로필 사진 - 💚 나중에 파이어베이스에서 가져오기
                                        "assets/flog_logo.png",
                                        width: 55,
                                        height: 55,
                                        alignment: Alignment.centerLeft,
                              ...
                              const SizedBox(width: 20),
                              const Text( //내 이름 - 💚 나중에 파이어베이스에서 가져오기
                                textAlign: TextAlign.left,
                                '사용자 2 - Me',
                                style: TextStyle(
                                    fontSize: 15,
